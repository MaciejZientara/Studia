...
def get_square(i):
    ...
    # 3 4 5
    # 6 7 8
    # kazdy kwadrat ma rozmiar 3x3, 
    # roznia sie pozycja w calej lamigowce (offset)
    down = int(i/3)
    right = i%3
    result = []
    for i in range(3):
        for j in range(3):
            result.append(V(i+3*down,j+3*right))
    return result
...
def sudoku(assigments):
    ...
    # w sudoku potrzebujemy zapewnic rozne cyfry w kazdym rzedzie, wierszu ale tez w kwadratach
    # na ktore podzielona jest plansza, np dla planszy 9x9 jest 9 kwadratow 3x3
